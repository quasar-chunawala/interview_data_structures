...
template <typename T> struct control_block_with_storage : control_block_base {
    ...
    explicit control_block_with_storage(Args&&... args)
        : control_block_base{}, m_object{T(std::forward<Args>(args)...)} {}
...
template <typename T> class shared_ptr {
  public:
    ...
    shared_ptr(T* t, control_block_base* cb) : m_raw_underlying_ptr{t}, m_control_block_ptr{cb} {}

    /* Parametrized constructor : Takes ownership of the pointee */
    shared_ptr(T* ptr) : m_raw_underlying_ptr{ptr} {
        try {
...
template <typename T, typename... Args> shared_ptr<T> make_shared(Args&&... args) {
    ...
    control_block_with_storage<T>* cb =
        new control_block_with_storage<T>(std::forward<Args>(args)...);
    T* data = cb->get();
